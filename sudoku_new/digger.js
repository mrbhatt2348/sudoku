class Digger {
    ...
    constructor(_level, _board, _boardSize) {
        ...
        this.LevelSelector = _boardSize == 16 ? 1 : 0;
    ...
    digChooser(howMany){
        let partition1 = this.random(howMany - 1) + 1;
    ...
    randomDig(howMany){
        let copyOfBoard = this.copyBoard(this.board);
        ...
        while (howMany > 0) {
            ...
            if(availabaleArray.length < 1){
                ...
                this.board = this.copyBoard(copyOfBoard);
                copyOfBoard = this.copyBoard(this.board);
            }else{
    ...
    symmetricalDig(howMany) {
        ...
        while(howMany > 0){
            let availabaleArray = this.availableCells();
            //if no items available for digging, start over
            if(availabaleArray.length < 1){
                howMany = copyOfHowMany;
                this.board = this.copyBoard(copyOfBoard);
                copyOfBoard = this.copyBoard(this.board);
            }else{
                let randomCell = availabaleArray[this.random(parseInt(availabaleArray.length/2 - 1))];
